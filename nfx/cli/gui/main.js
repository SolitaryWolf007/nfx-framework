window.addEventListener("load",function(){
	...
	requestmgr.onResponse = function(id,ok){ $.post("http://nfx/request",JSON.stringify({ act: "response", id: id, ok: ok })); }
	wprompt.onClose = function(){ $.post("http://nfx/prompt",JSON.stringify({ act: "close", result: wprompt.result })); }
	dynamic_menu.onClose = function(){ $.post("http://nfx/menu",JSON.stringify({ act: "close", id: dynamic_menu.id })); }
	dynamic_menu.onValid = function(choice,mod){ $.post("http://nfx/menu",JSON.stringify({ act: "valid", id: dynamic_menu.id, choice: choice, mod: mod })); }
